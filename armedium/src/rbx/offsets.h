...
// Fresh dump for version-2366ba214ec740ca from
// https://github.com/Offsetmanager/Roblox-Internal-External-Offsets
// (Windows/version-2366ba214ec740ca/External_Offsets.hpp).
// Version string lives in OffsetVersion.h — single source of truth.
namespace Offsets {
    ...
    namespace BasePart {
         ...
         // ── Compat aliases: code addresses primitive fields via BasePart.
         // Values mirror Primitive:: below for this client version.
         ...
         // Flag-bit helpers kept for code compatibility (see PrimitiveFlags).
    ...
    namespace Player {
         ...
         // NOTE: not present in the upstream 2366 dump; carried from
         // version-d584fb6c717a43d9 (0x380). Used by playerobjectscache.
         // NEEDS re-verifying on the new client before trusting GroupId.
    ...
    namespace PlayerMouse {
         ...
         // NOTE: Hit/Target/UnitRay are absent from the upstream 2366 dump;
         // carried from version-d584fb6c717a43d9. silentaim WRITES through
         // these, so re-verify on the new client before enabling silent aim.
         inline constexpr uintptr_t Hit = 0x120;
         inline constexpr uintptr_t Target = 0x60;
         inline constexpr uintptr_t UnitRay = 0xe0;
    }
    ...
    namespace Team {
         ...
         // NOTE: BrickColorName absent from upstream 2366 dump. BrickColor
         // moved 0xb8 -> 0xa8 (-0x10), so 0x238 = old 0x248 - 0x10 is an
         // ESTIMATE. Team ESP reads a string through this — re-verify before
         // trusting team checks on the new client.
         inline constexpr uintptr_t BrickColorName = 0x238;
    }
    ...
    namespace VisualEngine {
         // Dimensions verified live on version-2366ba214ec740ca: reads back
         // the exact game window size (1600x1024 matched to the pixel).
    ...
    namespace Workspace {
         ...
         // NOTE: upstream 2366 dump has no Workspace gravity field; World::
         // Gravity moved 0x210 -> 0x22c and old Workspace::Gravity equaled old
         // World::Gravity, so mirror the new World value. Re-verify — gravity
         // fly/mod WRITES through this offset.
